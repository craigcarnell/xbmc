...
void CDVDVideoCodecGStreamer::SetPictureParams(VideoPicture* pVideoPicture)
{
  ...
  auto colourPrimaries = gst_video_color_primaries_to_iso(m_pFrame->info.colorimetry.primaries);
  auto colorTransfer = gst_video_transfer_function_to_iso(info.colorimetry.transfer);

  pVideoPicture->color_primaries =  colourPrimaries == AVCOL_PRI_UNSPECIFIED
                                    ? static_cast<int>(m_hints.colorPrimaries)
                                    : colourPrimaries;

  pVideoPicture->color_transfer =   colorTransfer == AVCOL_TRC_UNSPECIFIED
                                    ? static_cast<int>(m_hints.colorTransferCharacteristic)
                                    : colorTransfer;

  auto colorSpace = gst_video_color_matrix_to_iso(info.colorimetry.matrix);

  pVideoPicture->color_space =
      colorSpace == AVCOL_SPC_UNSPECIFIED
      ? static_cast<int>(m_hints.colorSpace) : colorSpace;

  pVideoPicture->chroma_position = m_pFrame->info.chroma_site;
